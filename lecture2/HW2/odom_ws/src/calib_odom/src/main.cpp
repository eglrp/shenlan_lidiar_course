...
/* Create tf from pose2D */
Eigen::Matrix3d create_tf_from_post(Eigen::Vector3d pose)
{
    Eigen::Matrix3d A;
    float theta = pose(2);
    float x = pose(0);
    float y = pose(1);
    A << cos(theta), -sin(theta), x,
...
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    now_pos = odom_pose;
     //TODO:
    Eigen::Matrix3d tf_prv, tf_now, d_tf;
    
    tf_prv = create_tf_from_post(last_pos);
    tf_now = create_tf_from_post(now_pos);
    
    /* create realtive pose */
    d_tf = tf_prv.inverse()*tf_now;
    
    
    d_pos(0) = d_tf(0,2);
    d_pos(1) = d_tf(1,2);
    d_pos(2) = atan2(d_tf(1, 0), d_tf(0, 0));
   
    //printf("d_pos: X:%f Y:%f theta:%f\r\n", d_pos(0), d_pos(1), d_pos(2));
